...
class Database:
    ...
    def update_user_id_job(self, id_job, id):

        sql = f"UPDATE Users SET id_job=? WHERE id=?"

        return self.execute(sql, parameters=(id_job, id), commit=True)
    ...
    def add_recording(self, id_recording: int, date: str, status: str = None):
        sql = "INSERT INTO Diary_entries(id_recording, date, status) VALUES(?, ?, ?)"

        parameters = (id_recording, date, status)

        self.execute(sql, parameters=parameters, commit=True)

    def add_description(self, description, id_recording, date):

        sql = f"UPDATE Diary_entries SET description=? WHERE id_recording=? AND date=?"

        return self.execute(sql, parameters=(description, id_recording, date), commit=True)
    ...
    def select_record(self, **kwargs):
        sql = "SELECT * FROM Diary_entries WHERE "
        sql, parameters = self.format_args(sql, kwargs)

        return self.execute(sql, parameters=parameters, fetchall=True)

    def delete_diary_entries(self):
        self.execute("DELETE FROM Diary_entries WHERE TRUE", commit=True)

    def delete_last_record(self):
        self.execute("DELETE FROM Diary_entries WHERE id = (SELECT id FROM Diary_entries ORDER BY id DESC LIMIT 1)",
                     commit=True)
